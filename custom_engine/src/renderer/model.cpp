...
namespace Engine
{
    ...
    void CModel::LoadModel(const std::string &ct_ModelPath)
    {
        ...
        {
            ENGINE_CRITICAL_LOG("Failed to load aiScene.");
            ENGINE_ASSERT(0);
    ...
    // TODO: upgrade it somehow in order to detect transparency
    ...
    uint32_t CModel::TextureFromFile(const aiString *ct_TexturePath) const
    {
        ...
        switch (channels)
        {
            ...
            default:
            {
                ENGINE_CRITICAL_LOG("Unsupported texture format.");
                ENGINE_ASSERT(0);
